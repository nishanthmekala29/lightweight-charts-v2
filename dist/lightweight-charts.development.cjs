/*!
 * @license
 * TradingView Lightweight Charts™ v4.3.4-dev+202411051330
...
class PaneSeparator {
    constructor(chartWidget, topPaneIndex, bottomPaneIndex, disableResize, resizeDelegate) {
        ...
        this._private__topPaneIndex = topPaneIndex;
        this._private__bottomPaneIndex = bottomPaneIndex;
        this._private__resizeDelegate = resizeDelegate;
    ...
    _private__pressedMouseMoveEvent(event) {
        ...
        this._private__resizeDelegate._internal_fire(() => ({
            _internal_top: {
                _internal_index: this._private__topPaneIndex,
                _internal_height: this._private__paneA._internal_getSize().height,
            },
            _internal_bottom: {
                _internal_index: this._private__bottomPaneIndex,
                _internal_height: this._private__paneB._internal_getSize().height,
            },
        }));
    }
 ...
 */
function hitTestPaneView(paneViews, x, y, state) {
    for (const paneView of paneViews) {
...
class ChartWidget {
    constructor(container, options, horzScaleBehavior) {
        ...
        this._private__dblClicked = new Delegate();
        this._private__crosshairMoved = new Delegate();
        this._private__paneResized = new Delegate();
    ...
    _internal_paneResized() {
        return this._private__paneResized;
    }
    ...
    _private__syncGuiWithModel() {
        ...
        for (let i = actualPaneWidgetsCount; i < targetPaneWidgetsCount; i++) {
            ...
            if (i >= 1) {
                const paneSeparator = new PaneSeparator(this, i - 1, i, false, this._private__paneResized);
    ...
    _private__getMouseEventParamsImpl(index, details, event) {
        ...
        return {
            ...
            _internal_point: details && { x: details.x, y: details.y } || undefined,
...
class TimeScaleApi {
    ...
    _internal_timeToIndex(time, findNearest) {
        return this._private__timeScale._internal_timeToIndex(time, findNearest);
    }
    _internal_indexToCoordinate(index) {
        return this._private__timeScale._internal_indexToCoordinate(index);
    }
    coordinateToTime(x) {
        const timeScale = this._private__model._internal_timeScale();
    ...
    formatDateTime(time) {
        const timeScale = this._private__model._internal_timeScale();
        const internalHorzScaleTime = this._private__horzScaleBehavior.convertHorzItemToInternal(time);
        const timePointIndex = timeScale._internal_timeToIndex(internalHorzScaleTime, false);
        if (!timePointIndex)
            return "";
        const timePoint = timeScale._internal_indexToTimeScalePoint(timePointIndex);
        return timePoint ? timeScale._internal_formatDateTime(timePoint) : "";
    }
...
class ChartApi {
    constructor(container, horzScaleBehavior, options) {
        ...
        this._private__paneResizeDelegate = new Delegate();
        ...
        this._private__chartWidget._internal_paneResized()._internal_subscribe((paramSupplier) => {
            if (this._private__paneResizeDelegate._internal_hasListeners()) {
                this._private__paneResizeDelegate._internal_fire(this._private__convertPaneResizeParams(paramSupplier()));
            }
        }, this);
    ...
    subscribePaneResize(handler) {
        this._private__paneResizeDelegate._internal_subscribe(handler);
    }
    unsubscribePaneResize(handler) {
        this._private__paneResizeDelegate._internal_unsubscribe(handler);
    }
    removePane(index) {
    ...
    _private__convertPaneResizeParams(param) {
        return {
            time: param._internal_originalTime,
            top: {
                index: param._internal_top._internal_index,
                height: param._internal_top._internal_height,
            },
            bottom: {
                index: param._internal_bottom._internal_index,
                height: param._internal_bottom._internal_height,
            },
        };
    }
}
